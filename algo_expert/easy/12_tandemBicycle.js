/**
    ...
    bicycle moves at a speed of 5 (i.e., tandemSpeed = max(speedA, speedB)).
...
function tandemBicycle(redShirtSpeeds, blueShirtSpeeds, fastest) {
    ...
    for (let i = 0; i < redShirtSpeeds.length; i++) {
        result += fastest ? Math.max(redShirtSpeeds[i], blueShirtSpeeds[i]) :
            Math.min(redShirtSpeeds[i], blueShirtSpeeds[i])
    }
    return result;
}